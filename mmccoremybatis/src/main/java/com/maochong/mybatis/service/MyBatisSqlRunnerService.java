...
public class MyBatisSqlRunnerService {
    ...
    public void  selectOne(int id) throws Exception
    {
        ...
        if(map!=null && map.size()>0) {
            System.out.println(map.get("title".toUpperCase()));// key 会所有默认成大写
        }
        else {
        ...
        if(list !=null){
            list.forEach(x->{
                System.out.println(x.get("title".toUpperCase()));// key 会所有默认成大写
                System.out.println(x.get("date".toUpperCase()));// key 会所有默认成大写
                System.out.println(x.get("authorName".toUpperCase()));// key 会所有默认成大写
                System.out.println(x.get("content".toUpperCase()));// key 会所有默认成大写
            });
        }

        // 插入数据
        sqlRunner.setUseGeneratedKeySupport(true);// 设置这个返回的是自增量id大小
        int count = sqlRunner.insert("insert into blog(title,date,authorName,content) values(?,?,?,?)",new Object[]{"no","2018-03-31","jokin","hi"});
